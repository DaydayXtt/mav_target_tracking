...
class ViewManager:
    def __init__(self, 
        ...
        if self.video_flag is True:
            ...
            self.video = VideoWriter(  # 设置保存视频的一些参数
                video_name=video_name,
                bounding_box=(0, 25, 1850, 1200),
            ...
            if self.animation_flag:
                if self.map_flag:
                    ...
                    if self.planning_flag:
                        self.planner_vi1ewer = PlannerViewer(app=self.app)
    ...
    def update(self,
        ...
        if self.animation_flag: 
            if self.map_flag is True:
                self.mav_view.update(true_state, path, map, target_position, voro)
            elif self.waypoint_flag is True:
                self.mav_view.update(true_state, path, waypoints)
            elif self.path_flag is True:
                self.mav_view.update(true_state, path, target_position)
            else:
